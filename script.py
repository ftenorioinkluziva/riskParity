...
# Função para calcular estatísticas - corrigida para lidar com Multi-Índice
def calcular_estatisticas(dados_multi, nome, ticker):
    """
    Calcula estatísticas a partir dos dados históricos
    ...
    Returns:
        dict: Dicionário com as estatísticas calculadas ou None em caso de erro
    """
    try:
        # Verificar se os dados têm múltiplos índices
        if isinstance(dados_multi.columns, pd.MultiIndex):
            print(f"  Processando dataframe com multi-índice")
            
            # Extrair os dados para as colunas necessárias
            close_col = ('Close', ticker)
            
            # Verificar se a coluna existe
            if close_col not in dados_multi.columns:
                print(f"  ⚠️ Coluna {close_col} não encontrada no dataframe")
                close_cols = [col for col in dados_multi.columns if col[0] == 'Close']
                if close_cols:
                    print(f"  Usando coluna {close_cols[0]} em vez disso")
                    close_col = close_cols[0]
                else:
                    raise ValueError(f"Não foi possível encontrar coluna 'Close' para {ticker}")
            
            # Criar um novo dataframe com apenas os dados necessários
            dados = pd.DataFrame()
            dados['Close'] = dados_multi[close_col]
            
            # Calcular retorno diário
            dados['Retorno_Diario'] = dados['Close'].pct_change() * 100
            
            # Calcular peak e drawdown
            dados['Peak'] = dados['Close'].cummax()
            dados['Drawdown'] = (dados['Close'] / dados['Peak'] - 1) * 100
        else:
            # Se não for multi-índice, usa o dataframe diretamente
            dados = dados_multi.copy()
        
        # Agora processar as estatísticas com o dataframe normalizado
        retorno_acumulado = ((dados['Close'].iloc[-1] / dados['Close'].iloc[0]) - 1) * 100
        ...
        retorno_anualizado = ((1 + retorno_acumulado / 100) ** (365 / max(dias_totais, 1)) - 1) * 100
        ...
        info_ativo = {
            ...
            'sharpe': float(retorno_anualizado / volatilidade) if volatilidade > 0 else 0,
            'data_atualizacao': datetime.now().isoformat()
        }
        
        print(f"\nEstatísticas para {nome}:")
        print(f"  Preço atual: {info_ativo['preco_atual']:.2f}")
        print(f"  Retorno acumulado no período: {info_ativo['retorno_acumulado']:.2f}%")
        print(f"  Retorno anualizado: {info_ativo['retorno_anualizado']:.2f}%")
        print(f"  Volatilidade anualizada: {info_ativo['volatilidade']:.2f}%")
        print(f"  Máximo drawdown: {info_ativo['max_drawdown']:.2f}%")
        print(f"  Índice de Sharpe: {info_ativo['sharpe']:.2f}")
        
        return info_ativo
    except Exception as e:
        print(f"⚠️ Erro ao calcular estatísticas para {nome}: {str(e)}")
        return None
...
def preparar_dados_historicos(dados_multi, ticker, nome):
    ...
    try:
        ...
        if isinstance(dados_multi.columns, pd.MultiIndex):
            ...
            # Adicionar as colunas calculadas se existirem no dataframe original
            if hasattr(dados_multi, 'Retorno_Diario'):
                dados['Retorno_Diario'] = dados_multi['Retorno_Diario']
            if hasattr(dados_multi, 'Peak'):
                dados['Peak'] = dados_multi['Peak']
            if hasattr(dados_multi, 'Drawdown'):
                dados['Drawdown'] = dados_multi['Drawdown']
        else:
            # Se não for multi-índice, usa o dataframe diretamente
            dados = dados_multi.copy()
        ...
        # Calcular Retorno_Diario, Peak e Drawdown se não existirem
        if 'Retorno_Diario' not in dados.columns and 'Close' in dados.columns:
        ...
        if 'Peak' not in dados.columns and 'Close' in dados.columns:
            dados['Peak'] = dados['Close'].cummax()
            
        if 'Drawdown' not in dados.columns and 'Close' in dados.columns and 'Peak' in dados.columns:
            dados['Drawdown'] = (dados['Close'] / dados['Peak'] - 1) * 100
        ...
        for coluna in ['abertura', 'maxima', 'minima', 'fechamento', 'fechamento_ajustado', 
                     'volume', 'retorno_diario', 'pico', 'drawdown']:
...
def upsert_ativo(info_ativo):
    ...
    try:
        ...
        print(f"✅ Informações de {info_ativo['nome']} inseridas/atualizadas na tabela ativos")
        return True
    except Exception as e:
...
def atualizar_dados():
    ...
    for ticker, nome in ativos.items():
        try:
            ...
            if dados is not None and not dados.empty and len(dados) > 0:
                ...
                # Calcular estatísticas e preparar informações do ativo
                info_ativo = calcular_estatisticas(dados, nome, ticker)
                if info_ativo:
    ...
    if cdi_diario is not None and not cdi_diario.empty:
        ...
        print("Colunas disponíveis no CDI:", cdi_diario.columns.tolist())

        # Calcular estatísticas para o CDI
        info_cdi = calcular_estatisticas(cdi_diario, 'CDI', 'CDI')
        if info_cdi:
            # inspecionar o primeiro item do lote antes de enviar para Supabase:
            print("Primeiro item do lote antes de enviar para Supabase:")
            print(info_cdi)
            print("Inserindo/atualizando informações do CDI...")        
    ...
    # Descomente se quiser gerar arquivos CSV
    """
    print("\nExportando dados para CSV...")
    for ticker, dados in dados_ativos.items():
        try:
            nome_arquivo = f"{ticker.replace('.', '_').replace('=', '_')}_historico.csv"
            dados.to_csv(nome_arquivo)
            print(f"✅ Dados exportados para {nome_arquivo}")
        except Exception as e:
            print(f"⚠️ Erro ao exportar dados de {ticker}: {str(e)}")
    """
# Executar o script
